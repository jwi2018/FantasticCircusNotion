...
namespace EnhancedScrollerCustom.StageScroll
{
    public class StageScrollController : MonoBehaviour, IEnhancedScrollerDelegate
    {
        ...
        public RectTransform crossBanner = null;
        ...
        public float CellAdsHeight = 340f;

        public bool StageOpening = false;

        private void Awake()
        {
            cellViewPrefab.gameObject.SetActive(false);
            if (null != imageFrameADS)
            {
            ...
            if (crossBanner != null)
            {
                crossBanner.gameObject.SetActive(false);
            }
        }
        ...
        private void LoadData()
        {
            ...
            for (var i = 0; i < totalStage; i++)
            {
                ...
                if (false == _dicRows.ContainsKey(row))
                {
                    if (CellAdsHeight > 0)
                    {
                        if (row == lastlevelRow)
                        {
                            _dicRows.Add(row, CellAdsHeight);
                        }
                        //else if (row == lastRow)
                        //{
            ...
            _dicRows.Add(totalStage, CellAdsHeight);
            _data.Add(new StageScrollData()
            {
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScroller scroller, int dataIndex, int cellIndex)
        {
            ...
            bool isCrossBanner = true;

            if (crossBanner != null)
            {
                if (di >= MyLevel + 4)
                {
                    di = (dataIndex - 1) * numberOfCellsPerRow;
                    isCrossBanner = false;
                }
            }
            else
            ...
            cellView.SetData(ref _data, MyLevel, di, this, CellViewSelected, isCrossBanner);

            if (true == _dicRows.ContainsKey(dataIndex))
            {
                if (_dicRows[dataIndex] == CellAdsHeight)
                {
                    //Debug.LogWarningFormat("KKI {0}", dataIndex);
                    if (null != corAD)
                        StopCoroutine(corAD);
